...
class SearchApis {
  ...
  static Future<User?> searchUser(String username) async{
    ...
    if(token != "") {
      ...
      if (logged_in_username != username) {
        var uri = Uri.parse("$baseUrl/user/getuserbyusername/$username?jwtkey=$token");
    print(uri.toString());
  ...
  if(response.statusCode == 200) {
   ...
   log(jsonUser.toString());
    ...
    Get.snackbar('Error whie Searching :/', resDecode['error'], backgroundColor: Colors.blue, snackPosition: SnackPosition.BOTTOM, colorText: Colors.white);
    return null;
  }
      }
      else {
      ...
      Get.snackbar('Are you Serious?🙄', 'You cannot search yourself', backgroundColor: Colors.blue, snackPosition: SnackPosition.BOTTOM, colorText: Colors.white);
